...
/**
 * Converts an uploaded image File into a compressed Blob suitable for uploading
 * to object storage (e.g. Supabase Storage). Downscales to a sensible max
 * dimension to keep the upload small. PNGs are preserved as PNG (transparency);
 * everything else is encoded as JPEG.
 *
 * Returns the compressed blob and the matching file extension. Falls back to
 * the original file if the canvas pipeline is unavailable.
 */
